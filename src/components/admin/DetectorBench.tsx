 ...
 * request.auth.token. The role IS enforced, but by a get() in the rules and by
 * requireAdmin() on the API routes, not by this hook. So everything here is
...
export function DetectorBench() {
  ...
  // precisely because this bench is a scratchpad: the change lives in this tab
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <p className="pb-2 text-center text-xs leading-relaxed text-ink-600">
        Le banc ne touche à rien : il lit la caméra et affiche des mesures. Les
        seuils modifiés ici vivent dans cet onglet et se recopient à la main
        dans le code.
      </p>
    </div>
  );
